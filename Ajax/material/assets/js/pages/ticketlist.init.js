...
$.ajax({
    url: "assets/json/support-tickets-list.json",
    cache: false,
    dataType: "JSON",
    type: "GET",
    success: function (data) {
        data.forEach((raw) => {
            ticketsList.add({
                id: '<a href="javascript:void(0);" onclick="ViewTickets(this)" data-id="' + raw.id + '" class="fw-medium link-primary ticket-id">#VLZ' + raw.id + "</a>",
                tasks_name: raw.tasks_name,
                client_name: raw.client_name,
                assignedto: raw.assignedto,
                create_date: str_dt(raw.create_date),
                due_date: str_dt(raw.due_date),
