<script>
	...
	function activateParentDropdown(item) {
		...
		if (parentCollapseDiv) {
			...
			if(parent) {
				parent.querySelector("a[data-bs-toggle]").setAttribute("aria-expanded","true"); // 1st parent dropdown
				if (parent.closest(".collapse.menu-dropdown")) {
					...
					if(parentParent) {
						parentParent.querySelector("a[data-bs-toggle]").setAttribute("aria-expanded","true"); // 2nd parent dropdown
						if (parentParent.closest(".collapse.menu-dropdown")) {
							...
							if(parentParentParent) {
								parentParentParent.querySelector("a[data-bs-toggle]").setAttribute("aria-expanded","true"); // 3nd parent dropdown
							}
						}
					}
				}
			...
			<div class="collapse menu-dropdown {current === item.id ? 'show' : ''}">
				<ul class="nav nav-sm flex-column">
					{#each item.subItems as subItem}
						...
						{:else}
							<li class="nav-item">
								<a class={current_child === subItem.id ? "nav-link active" : "nav-link"}
								...
								<div
									class="collapse menu-dropdown {current_child === subItem.id ? 'show' : ''}"
								>
									<ul class="nav nav-sm flex-column">
										{#if subItem.isChildItem}
											{#each subItem.childItems as childItem}
												...
												{:else}
													<a class={current_subchild === childItem.id ? "nav-link active" : "nav-link"}
														id={childItem.id}
														data-bs-toggle="collapse"
													...
													<div class="collapse menu-dropdown {current_subchild === childItem.id ? 'show' : ''}" aria-expanded={current_subchild === childItem.id ? true : false}>
														<ul class="nav nav-sm flex-column">
