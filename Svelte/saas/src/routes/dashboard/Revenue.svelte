<script>
	import { Card, CardHeader, Row, Col, CardBody } from 'sveltestrap';

	import RevenueCharts from './DashboardEcommerceCharts.svelte';

</script>

<Card>
	<CardHeader class="border-0 align-items-center d-flex">
		<h4 class="card-title mb-0 flex-grow-1">Revenue</h4>
		<div>
			<button type="button" class="btn btn-soft-dark btn-sm"> ALL </button>
			<button type="button" class="btn btn-soft-dark btn-sm"> 1M </button>
			<button type="button" class="btn btn-soft-dark btn-sm"> 6M </button>
			<button type="button" class="btn btn-soft-primary btn-sm"> 1Y </button>
		</div>
	</CardHeader>

	<CardHeader class="p-0 border-0 bg-light-subtle">
		<Row class="g-0 text-center">
			<Col xs={6} sm={3}>
			<div class="p-3 border border-dashed border-start-0">
				<h5 class="mb-1"><span class="counter-value" data-target="7585">0</span></h5>
				<p class="text-muted mb-0">Orders</p>
			</div>
			</Col>
			<Col xs={6} sm={3}>
			<div class="p-3 border border-dashed border-start-0">
				<h5 class="mb-1">$<span class="counter-value" data-target="22.89">0</span>k</h5>
				<p class="text-muted mb-0">Earnings</p>
			</div>
			</Col>
			<Col xs={6} sm={3}>
			<div class="p-3 border border-dashed border-start-0">
				<h5 class="mb-1"><span class="counter-value" data-target="367">0</span></h5>
				<p class="text-muted mb-0">Refunds</p>
			</div>
			</Col>
			<Col xs={6} sm={3}>
			<div class="p-3 border border-dashed border-start-0 border-end-0">
				<h5 class="mb-1 text-success">
					<span class="counter-value" data-target="18.92">0</span>%
				</h5>
				<p class="text-muted mb-0">Conversation Ratio</p>
			</div>
			</Col>
		</Row>
	</CardHeader>

	<CardBody class="p-0 pb-2">
		<div class="w-100">
			<div dir="ltr">
				<RevenueCharts dataColors={["--vz-success", "--vz-info" , "--vz-danger" ]} />
			</div>
		</div>
	</CardBody>
</Card>