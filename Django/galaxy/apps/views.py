...
def apps_job_application_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request,"Something went wrong!")
            return redirect('apps:job.application')
    return render(request,'apps/job/apps-job-application.html',{'applications':applications})

def apps_job_update_application_view(request,pk):
    applications = JobApplication.objects.get(pk=pk)
    if request.method == 'POST':
        form = JobApplicationForm(request.POST or None, request.FILES or None,instance=applications)
        if form.is_valid():
            form.save()
            ...
            messages.error(request,'Something went wrong!')
            return redirect("apps:job.application")
    return render(request,'apps/job/apps-job-application.html')
...
def apps_ecommerce_update_orders_view(request,pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(request,"Order updates Successfully!")
            return redirect("apps:ecommerce.orders")
        else:
            messages.error(request,"Something went wrong!")
            return redirect("apps:ecommerce.orders")
...
def apps_ecommerce_update_customers_view(request,pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(request,"Something went wrong!")
            return redirect("apps:ecommerce.customers")
    return redirect(request,"apps/ecommerce/apps-ecommerce-customers.html")
...
def apps_tickets_list_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(request,"Ticket inserted Successfully!")
...
def apps_tickets_update_list_view(request,pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(request,"Something went wrong!")
            return redirect("apps:tickets.list")
    return redirect(request,"apps/support-tickets/apps-tickets-list.html")
