...
from . models import CrmCompany,CrmContact,CrmLead,JobApplication,EcommerceCustomer,EcommerceOrder,TicketList
from . forms import *
...
def apps_job_application_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request,"Something went wrong!")
            return redirect("apps:job.application")
    return render(request,"apps/job/apps-job-application.html",{'apps':apps})
...
def apps_ecommerce_orders_view(request):
    order = EcommerceOrder.objects.all().order_by('-id')
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect("apps:ecommerce.orders")
    return render(request,'apps/ecommerce/apps-ecommerce-orders.html',{'orders':order})
...
def apps_tickets_list_update_view(request,pk):
...
def apps_tickets_list_delete_view(request,pk):
    tickets = TicketList.objects.get(pk=pk)
    tickets.delete()
