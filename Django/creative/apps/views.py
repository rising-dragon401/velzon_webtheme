...
from . models import CrmCompany,CrmContact,CrmLead,JobApplication,EcommerceOrder,EcommerceCustomer
from . forms import *
...
apps_job_candidate_list_view = AppsView.as_view(template_name="apps/jobs/apps-job-candidate-lists.html")
apps_job_companies_list_view = AppsView.as_view(template_name="apps/jobs/apps-job-companies-lists.html")
apps_job_categories_view = AppsView.as_view(template_name="apps/jobs/apps-job-categories.html")
apps_job_detail_view = AppsView.as_view(template_name="apps/jobs/apps-job-details.html")
apps_job_grid_list_view = AppsView.as_view(template_name="apps/jobs/apps-job-grid-lists.html")
apps_job_list_view = AppsView.as_view(template_name="apps/jobs/apps-job-lists.html")
apps_job_new_view = AppsView.as_view(template_name="apps/jobs/apps-job-new.html")
apps_job_statistic_view = AppsView.as_view(template_name="apps/jobs/apps-job-statistics.html")
...
def apps_crm_update_contacts_view(request,pk):
    ...
    if request.method == "POST":
        form = CrmContactUpdateForm(request.POST or None,request.FILES or None,instance=contacts)
        if form.is_valid():
            form.save()
            ...
            messages.error(request,"Something went wrong!")
            return redirect("apps:crm.contacts")
    return render(request,'apps/crm/apps-crm-contacts.html')
...
def apps_crm_update_leads_view(request,pk):
    ...
    if request.method == "POST":
        form = CrmLeadsUpdateForm(request.POST or None,request.FILES or None,instance=leads)
        if form.is_valid():
            form.save()
